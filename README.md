...
```
**Note:** The development version includes a new function, `download_from_ogimet()`
...
* **Wind:** Direction (`dd`) in **tens of degree** and speed (`ff`) with its unit.
* **Temperature and humidity:** Air temperature (`TTT`) and dew point (`TdTdTd`) in **°C**. Relative humidity is derived from both.
* **Pressure:** Mean sea level pressure (`PPPP`) and surface pressure (`PPPP1`) in **hPa**. Also, geopotential height (`hhh`) in **gpm**.
