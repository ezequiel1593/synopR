...
Why are some meteorological parameters not completely decoded? Is because it won't produce a strictly numeric vector, or because the output would be too long.

* Horizontal visibility `VV`, for example, has codes 00 (< 100 m), 89 (> 70 km)
* Lowest cloud base height `h` is expressed in intervals (e.g., `h` = 0 means "between 0 and 50 meters")
* Cloud cover `N` and `Nh` can be directly interpreted as in oktas (octaves), except when it's 9, which mean the sky is not visible due to fog or other meteorological phenomenon
* Some of them consist of long descriptions: present and past weather `ww`, `W1`, `W2`,  cloud-related `Cl`, `Cm` and `Ch`. ground-related `E` and `E'` 
...
* Snow depth `sss` is assumed to be between 1 cm and 996 cm. '997' means 'less than 0.5 cm'.
